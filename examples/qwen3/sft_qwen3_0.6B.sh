...
# ============ Batch Size 配置 ============
BATCH_SIZE=8                     # $3  单个数据并行内的样本数 (micro batch size)
GLOBAL_BATCH_SIZE=8            # $4  所有数据并行的总样本数 (global batch size)
# ============ 训练超参 ============
LR=1e-5                          # $5  学习率
MIN_LR=1e-6                      # $6  最小学习率
SEQ_LEN=128                      # $7  序列长度
...
# ============ 训练模式 ============
SFT=true                         # $18 是否微调训练: true(SFT), false(预训练)
# ============ 训练进度 ============
TRAIN_TOKENS_OR_ITERS=1000     # $25 训练 Token 数或迭代数, 预训是Tokens，SFT是迭代数 下同
WARMUP_TOKENS_OR_ITERS=100      # $26 预热 Token 数或迭代数
SAVE_INTERVAL=10000              # $21 保存 checkpoint 间隔 (迭代)
# ============ 数据路径 ============
DATASET_PATH=/mnt/qwen-datasets/alpaca_zh-train-general.json       # $22 训练数据集路径
VALID_DATASET_PATH=/mnt/qwen-datasets/alpaca_zh-valid-general.json # $23 验证数据集路径
# ============ 模型路径 ============
PRETRAIN_CHECKPOINT_PATH=/mnt/qwen-ckpts/Qwen3-0.6B         # $24 预训练模型路径
# ============ 输出路径 ============
OUTPUT_BASEPATH=/mnt/logs/output_mcore_qwen3_sft                   # $27 训练输出日志文件路径
# ============ 执行训练 ============
