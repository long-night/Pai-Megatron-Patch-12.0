...
from megatron.training.utils import print_model_weight,print_model_data


def print_time(tag):
    # 绝对时间，精确到毫秒
    t = time.time() * 1000
    print(f"[Rank {torch.distributed.get_rank()}] {tag}: {t:.3f}", flush=True)
